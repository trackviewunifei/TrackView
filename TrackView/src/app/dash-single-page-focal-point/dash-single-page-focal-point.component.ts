...
@Component({
  selector: 'app-dash-single-page-focal-point',
  templateUrl: './dash-single-page-focal-point.component.html',
  styleUrls: ['./dash-single-page-focal-point.component.css']
})
export class DashSinglePageFocalPointComponent implements OnChanges {
  ...
  private async getDash(){
    ...
    //This  analysis is based on a different point of view, so its necessary a conversion
  ...
  private cardInsertData(){
    ...
    //@ts-ignore
    this.cardAjust(this.configs.Cards[0].Title, (100*medCoherence).toFixed(2), this.configs.Cards[0].Text_1, (this.calcDeviationCoherence(medCoherence, totalAreas)*100).toFixed(2), this.configs.Cards[0].Text_2, 1);
    //@ts-ignore
    ...
    //@ts-ignore
    this.cardAjust(this.configs.Cards[3].Title, (medChoose).toFixed(2), this.configs.Cards[3].Text_1, this.calcDeviationCoherence(medCoherence, totalAreas).toFixed(2), this.configs.Cards[3].Text_2, 4);
  ...
  private calcDeviationTime(medium, total){
    var deviation = 0;
    this.areasData.forEach(element => {
      deviation += Math.pow(element["Time"] - medium , 2);// balance the minutes, because timeMed is in milliseconds
    }); 

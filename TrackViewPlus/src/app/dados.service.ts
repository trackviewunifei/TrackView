...
export class DadosService {
    ...
    public async insertData(query: string) {
        var response;
        await this.neo4j
            .connect(
                this.url,
    ...
    public async removeWrongNode(query: string) {
        var response;
        await this.neo4j
            .connect(
                this.url,
    ...
    public async getEventsFromNeo4j(query: string) {
        //console.log('tamo aqui getEventsFromNeo4j')

        //if the funcition was alread called before
        if (this.eventsQuery == query) {
            //console.log('recall')
            return this.ajustedData;
        }
        ...
        //console.log('dados ajustados: ', this.ajustedData)
        return this.ajustedData;
    }

    //Ajust the events for each client
    private async getMultiPagesEvents() {
        //console.log('tamo aqui getMultiPagesEvents')
        this.ajustedData = new Array();
        var obj, arr: any = [];
    ...
    private convertObjectivePage(page_tag: string) {
        var page = page_tag.split('|')
        var pageSplited = page[0].split('/') 
        if(
            pageSplited[0] == 'localhost' || 
            pageSplited[0] == 'localhost:8100'
        ){
            var newPage = 'Outras'
        }
        //else if(pageSplited[0] == 'www.linkmotors.com.br' && 
        //        pageSplited[1] != 'anuncio'){
        //    var newPage = 'Outras'
        //}
        else if(pageSplited[0] == 'www.linkmotors.com.br' && 
                pageSplited[1] == 'anuncio'){
            var newPage = //pageSplited[0] + '/' + 
                          '/' + pageSplited[1] //+ '/' 
                          //pageSplited[3]
        }
        else if(pageSplited[1] != undefined){
            var newPage = //pageSplited[0] + 
                            '/' + pageSplited[1]
        }
        else if (pageSplited[0] == 'www.linkmotors.com.br' || pageSplited[0] == 'linkmotors.com.br' &&
                 pageSplited[1] == undefined){
            var newPage = 'linkmotors/'
        }
        else if (pageSplited[1] == undefined){
            var newPage = pageSplited[0] + '/'
        }
    ...
    private getPages(elements: any[]) {
        ...
        elements.forEach(element => {
            page = this.convertObjectivePage(element[1]);//orignally wass convert_page_name
            //page = element[1]
    ...
    private getTotalTime(elements: any[]) {
        //console.log('tamo aqui getTotalTime')
        var time = 0;
        var data1, data2;
    ...
    private divideEvents(clientData: any[]) {
        //console.log('tamo aqui divideEvents')
        var events: any[] = []
        var i, j;
        var obj, data: string[];

...
@Component({
  selector: 'app-dash-clientes-eventos',
  templateUrl: './dash-clientes-eventos.component.html',
  styleUrls: ['./dash-clientes-eventos.component.css']
})
export class DashClientesEventosComponent implements OnChanges {
            ...
            element["Events"].forEach(event => {
                this.lineChart.forEach(data => {
                    ...
                    if (event["Date"].includes(date[1] + "-" + date[0])) {
                        //if (element["Pages"].includes("Checkout"))
                            data[1] += event["EventsData"].length;
                            totalEventos = totalEventos + event["EventsData"].length
                        //else
